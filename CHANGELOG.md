...
- **A sealed program has no way to ask for the time.**
  ...
  `doc/Determinism.md` has the design and one deadline worth knowing
  about: a `"pending"` status must be reserved in the hostcall result
  encoding before the first hostcall ships, or adding an async one later
  is a version break.
