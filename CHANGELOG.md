...
`v0.16.1` &middot; Lua 5.5.1 &middot; bytecode format `0x46`

**The dev-build machinery, and nothing else. The runtime is byte-for-byte
0.16.0's: no source file changed, the dv ABI is still 2, and a program
that ran on 0.16.0 runs here identically.**

This release exists so the released line keeps moving while a breaking
change is developed elsewhere. `doc/ALIGNMENT.md` §7's dev builds were
specified and never wired: no workflow carried a `schedule:` trigger,
`build.yml` had no push trigger and its only caller was `release.yml` on
a `v*` tag, and no `v*-dev.*` tag had ever been cut. Merging ran the
tests and published nothing.

**A consumer that tracks this repository by git can take this safely.**
That is the point of it: the ABI 3 work lives on `release/0.18.0` and
reaches `main` only when it ships, so a spot fix never arrives welded to
a breaking change.

### Added

- **Dev builds and nightlies (`doc/ALIGNMENT.md` §7).**
  `script/dev-tag.sh` allocates the next `v<version>-dev.<n>` from the
  tags that exist rather than from a counter in the tree, so a nightly
  commits nothing and two branches cannot collide; `make dev-tag` prints
  it. `nightly.yml` builds nothing itself -- it allocates a number and
  dispatches `release.yml`, on push to `release/**`, on a cron that
  skips an unmoved HEAD, and on dispatch.

  The counter is global and never reused, so a number names one build
  forever and does not restart when the version base moves.
- **`doc/ALIGNMENT.md` §7 gains "Where a dev build is cut from, and why
  not `main`".** The rule stated generally: a repository other projects
  track by git keeps `main` releasable. A git dependency updates all or
  nothing, so a `main` carrying an unreleased break means a consumer
  wanting a one-line fix can only have it by porting to the break --
  frozen rather than protected, and the two are indistinguishable until
  someone needs the fix.

### Changed

- **`release.yml` branches on the tag suffix.** A `v*-dev.*` tag skips
  the changelog's release-check (a dev build has no entry, by design),
  is always a prerelease, takes its notes from the newest entry, and is
  titled by its tag. `build.yml` gains a `dev` input that skips macOS,
  Windows, the ARM builds and both wasm builds -- one platform, as §7
  asks. A dev build takes about seven minutes against a full release's
  thirty-five.
- **`docker.yml` no longer fires on a dev tag.** An image per nightly is
  noise, and §7's retention argument applies to a registry the way it
...
- **§7's own `tags-ignore` snippet does not work**, and the other
  ...
  negative pattern inside `tags:` here instead. §7 is not yet corrected.
- **A dev build's binary reports the tree's version, not its tag.**
  `DILUVIUM_BUILD` is compiled from the `VERSION` file, so two dev
  builds cut from one tree report the same string. Does not affect this
  release, whose version and tag agree.

### Upgrading

**Nothing to do.** No source file changed between 0.16.0 and this
release, so the binaries differ only in the version string they report.
The dv ABI is 2, unchanged; snapshots, bytecode and every binding work
exactly as they did.
...
**The numeric round: a typed array tier whose float results are the
same bits on every target, fourteen new syntax forms, and the last of
the ambient nondeterminism a pure program could observe.**

The language gains default parameters, numeral separators and `0b`
literals, `const`, spread, expression-bodied functions, lambdas,
destructuring, `if` and `switch` as expressions, slicing, `export` and
classes -- fourteen forms across three tiers, every one a parse error
in stock Lua 5.5, none of them a reserved word, none of them an opcode.
(`continue` landed in `0.15.1` and is unchanged.)

Alongside it, an `array` type behind the `numeric` build feature, with
kernels whose answers are bit-identical across x86-64, aarch64, wasm
and Windows, and the libm that makes that true of `math` as well. And
two things a program could watch vary from one run to the next that it
no longer can: the order `pairs` visits table, function, userdata and
coroutine keys in, and the result of `^`.

### Added

- **A numeric tier: the `array` type and the kernels over it.** Typed,
  1D and 2D, strided views, `f64`/`i64`/`u8`/`c128`, with elementwise
  and reduction kernels, sorting, grouping, linear algebra, and the
  transforms -- `fft`/`ifft`, `rfft`/`irfft`, `ntt`/`intt`, and
  `convolve`/`correlate`, which goes through the NTT and stays exact
  when both operands hold integers. A slice of an array is a view.

  Behind the `numeric` build feature: off in a default build, on in the
  release builds. `dv_features()` is what a host should ask.
- **Float results that are bit-identical on every target.** The
  reproducible tier is not a promise about a platform, it is a property
  the build enforces: a canonical eight-accumulator reduction order, a
  total order with NaN last, first-appearance group ids, a fixed
  SplitMix64 mix, and openlibm's stage-1 subset vendored under `dv_`
  prefixes so a transcendental does not depend on which libm the
  platform ships -- `math.exp` and its neighbours, and every kernel.
  `test/numeric/corpus.lua` prints 103 lines of IEEE bit patterns and
  CI diffs them across x86-64 and aarch64.

  One limit, stated where the kernels are: IEEE 754 does not interpret
  the sign of a NaN, so a program that computes one and then reads its
  bit pattern reads its libm's choice. Where an answer has to be the
  same everywhere the NaN is canonicalised rather than carried.
- **The `^` operator is one of them.** The vendored libm is installed
  over the `math` table, but `^` compiles to `OP_POW` and never looks
  at `math`, so it reached the platform's `pow` -- which disagrees with
  the vendored one in the last bit on 2,837 of 30,000 sampled pairs.
  Scalar `^`, the array kernel's `^`, and a `^` the compiler folds at
  compile time now all go through `dv_pow`, so two spellings of one
  expression give one answer on one build, and the corpus diffs a line
  for it target against target.
- **`pairs` order is a property of the program, for every key a program
  can make.** A table, closure, full userdata or coroutine used as a
  key hashes by an identity it takes when it is created -- a per-state
  counter -- and not by its address. Stock Lua hashes those by address,
  so under ASLR the same program visited the same keys in a different
  order on every run, and a different order again on every platform:
  the hole the fixed string hash seed closed for string keys, still
  open for every other kind, and silently so. Light userdata and C
  functions have no header to carry an identity and stay by address; a
  light userdata is a host pointer by definition.
- **Tier A syntax.** Default parameters, numeral separators and `0b`
  literals, `const`, spread, and expression-bodied functions.
- **Tier B syntax.** Lambdas, destructuring, `if` and `switch` as
  expressions, slicing, and `export`.
- **Classes.** `class NAME [extends exp] ... end`, with field defaults
  evaluated per instance, `function` methods taking an implicit `self`,
  `static` to opt out, `super(...)` and `super.m(...)`, and `@name` for
  `self.name`. The desugar is plain metatables through `dv.class`, so a
  class is a table and there is nothing new in the object model.
- **`dv_array_adopt`**, the zero-copy handover: a host buffer becomes an
  `array`'s elements with no copy at all, or a string where the feature
  is off. The adopted bytes count against the instance's memory limit
  from the moment it takes them, and a handover the instance cannot
  take is refused with `dv_last_error` set rather than aborting.
- **`test/test_determinism.lua` pins what was always true of
  `math.random`.** The `"DILU"` constant that fixes the string hash
  seed is also what `luaL_makeseed` returns, so a fresh state's first
  draw is the same number everywhere and `math.randomseed()` with no
  arguments does not consult the clock. Nothing in the suite held that;
  now the seed it reports and the first two draws are compared across
  processes and against recorded values.

### Changed

- **`dv_abi` is 2.** `dv_features`, `dv_build`, `dv_array_adopt` and the
  `dv_numeric_*` block are new surface, so a wrapper built against 1
  does not know what it is holding and is refused rather than allowed to
  misread it.
- **`tostring` prints an identity, not an address.** `table: #42`,
  `function: #17`, `thread: #3` -- the same per-state creation counter
  `pairs` hashes by -- rather than `table: 0x55e6db854690`, which was
  a different string on every run and, on Windows, a different shape
  (`%p` there prints no `0x`). A value with no identity to print keeps
  its pointer: light userdata and light C functions, so `tostring(print)`
  is as it was. A `__tostring` metamethod is untouched, and so is
  `string.format("%p")`, which is the way to ask for the address on
  purpose. Hosts read the same number through `lua_objectid`.
- **A table is eight bytes larger on 64-bit targets** (`sizeof(Table)`
  56, from 48). The key identity is a four-byte field, and `Table` was
  the one type with no padding to put it in; closures, userdata and
  threads absorbed theirs.
...
- **`dv_build()` outlived the counter it reports.** It was added to the
  ABI-2 surface as the `N` in `5.5.1_buildN`; `0.15.1` retired build
  numbers, so it returns a hardcoded value with nothing left to check
...
**Nothing to do for a program.** Every new form is syntax that was a
syntax error before, no existing construct changed meaning, and the
bytecode format byte does not move. Two results can differ from
`0.15.1` on the same machine: a `^` between non-trivial floats can move
by one ulp (it used to move by one ulp between machines instead), and a
loop over a table keyed by objects now sees creation order -- a program
that depended on the old order was depending on the allocator. And
`tostring` of a table, function or coroutine reads `table: #N` rather
than `table: 0x...`; a program that scraped the hex out of it was
scraping an address, and `string.format("%p")` still hands one over.

**A host built against dv ABI 1 is refused by `dv_new`, and a snapshot
taken by one will not restore here.** That is the ABI bump doing its
job. Rebuild the host against this `dv.h`; drain or replay snapshots
before upgrading a deployment that holds them.


## [0.15.1] - 2026-09-16

`v0.15.1` &middot; Lua 5.5.1 &middot; bytecode format `0x46`
...
number -- `0.15`, the fourteen builds carried forward as fourteen
...
- **`_DILUVIUM`, a version global.** A table the runtime installs:
  `version` is Diluvium's own (`0.15.1`), `lua` the Lua base it forks
...
and a snapshot crosses build14 and 0.15.1 in both directions. What
changes is spelling. The release tag is `v0.15.1`, not `v5.5.1_build15`;
`_DILUVIUM.version` reads `0.15.1`; and the Lua base is `_DILUVIUM.lua`
...
published and needs a branch for `0.15.x`-shaped tags going forward.

(`v0.15.0` was tagged first, for the same tree; a stuck tag ref meant CI
could not cut a release for it, so `0.15.1` is the published cut. The
`v0.15.0` tag carries no release.)


## [5.5.1_build14] - 2026-09-12

`v5.5.1_build14` &middot; Lua 5.5.1 &middot; bytecode format `0x46`

**`DV_FLAG_TEXT_ONLY` now covers the guest's own `load`.**
The flag refused a precompiled chunk at `dv_load` and nowhere else, so
a program running under it could still hand `load` a binary chunk --
the same bytes entering the same VM by a second door, one the host
that asked for source only was not watching. That is the whole of this
release: build13's content plus one fix, no new surface, no ABI or
format movement.

### Changed

- **The Rust binding's `Config::text_only` no longer documents the
  defect as the design.** Its doc comment said the flag "binds
  `dv_load` and not the program's own `load()`", which was an accurate
  description of the hole. It now says both doors, and names the
  `load(string.dump(f))` casualty. Comment only; the crate passes the
  flag through and always did.

### Security

- **`DV_FLAG_TEXT_ONLY` did not reach the guest's own `load`.** The flag
  exists because the loader's operand checks are not a verifier -- Lua
  5.1 shipped a fuller one and still had escapes -- so a host that did
  not compile the bytes can refuse precompiled chunks outright. It
  refused them at `dv_load` only. The guest's `load` took a binary
  chunk regardless, and a guest can make one with `string.dump` or out
  of a string literal, so "source only" held at one door and not the
  other. `dlibs.c` had said as much in a comment beside `seal_base`,
  filed it under this flag, and left it; this is the flag's answer.

  `dv_new` now maps the flag to `DILUVIUM_GUEST_TEXT_ONLY`, and
  `diluvium_openguestlibs` replaces `load` with a closure over the real
  one that forces the mode to `"t"`. A wrapper rather than an edit to
  `luaB_load`, so `lbaselib.c` stays the stock file and merging a
  future Lua release stays a merge of upstream's diff. It applies
  sealed or not: a host sets the flag because it did not compile the
  bytes, which is as true of an unsealed instance as of a sealed one,
  and the stdlib seal already has its own flag.

  `text_only_reaches_the_guests_own_load` in `test/dv_check.c` holds
  it. The chunk it feeds `load` is built with `string.dump`, so the
  bytes are well-formed and made by this very VM -- a malformed chunk
  would be caught by the loader's own checks and prove nothing -- and
  the second half asserts that without the flag a dump still loads,
  which is stock Lua and stays.

  What it does not close: under `DV_FLAG_UNSAFE_STDLIB` the `package`
  searchers can still reach a precompiled *file*. That flag's own
  documentation is that an instance carrying it stops being a
  boundary, so closing one hole in a wall that has a door would be
  tidying. Recorded so that "both doors" is not read as "every door".

### Known issues

- **Carried forward from 5.5.1_build12p1, because it is still true: a
  guest that catches its budget error in a loop still spins.** build12p1
  bounded the *work* a guest can do past its budget; it did not make the
  error uncatchable and does not return control to the host.
  ...
  Closing it needs a `pcall`/`xpcall` that refuses to catch once
  `exceeded` is set, which is a core-file patch and so a
  `CORE_PATCH_ALLOWLIST` decision, or a process-level watchdog, since a
  host inside `dv_run` has no way in. The practical bound today is
  operational: run the host under a supervisor that restarts it, and
  alert on the restart count rather than absorbing it.

### Upgrading

Nothing to do for a host, and no format movement. `DV_ABI_VERSION`
stands, `LUAC_FORMAT` stays `0x46`, and the snapshot permanents
fingerprint is unmoved: it is a hash over the sorted *names* the
permanents walk finds, `load` keeps its name and only the function
under it changes, so a snapshot crosses build13 and build14 in both
directions. The restoring runtime's `load` is the one a woken program
gets, which is the behaviour to want.

One behaviour change, and only under the flag: `load(string.dump(f))`
is refused, a round trip through bytecode the VM produced itself. That
is deliberate. Nothing can tell that string from one a program
assembled byte by byte, so a host that asked for source gets source;
`string.dump` itself still works and its output still crosses a queue
to a host that wants it. A program that relied on reloading its own
dump under `DV_FLAG_TEXT_ONLY` was relying on the defect. A host that
does not set the flag sees stock Lua, as before -- and that includes
every instance in a swarm, and so `diluvium-host`: the swarm layer's
one flag authority is `dvs_allow_unsafe_stdlib`, which never sets this
one. The Python binding's `from_source` and the JS binding's default
(`allowBytecode: false`) do set it, so programs under those see the
change.


## [5.5.1_build13] - 2026-09-06
...
- **The per-agent memory figures are re-measured, and one of them had
  drifted 23%.** `doc/Benchmarks.md` advertised ~73 KB of guest heap per
  awake agent and ~14,600 agents per GiB; the same harness, unchanged
  since those numbers were written, now reports **~90 KB and ~11,700**.
  About 1 KB of that is this release's `regex` library. The rest
  accumulated across earlier builds and nobody re-ran the benchmark, so
  the README, the guide and the benchmark doc had all been reprinting a
  figure that stopped being true some builds ago. Corrected in all four
  places. The hibernated figures did not move -- still ~1.4 KB and
  ~750,000 per GiB -- because a snapshot carries program state and not
  the library tables an instance is handed.

  Only the byte and count rows were re-measured; the timings in that
  table are still the reference machine's, because `swarm_bench` says
  byte figures are comparable across machines and times are not.
...
- **Listener response headers.** A reply may carry `headers = { ... }`,
  gated per listener by `response_headers` -- a lowercase allowlist
...
- **Rest plugin response headers.** `rest/get` and `rest/post` results
  carry a `headers` map: names lowercased, repeats joined ", ", at
  ...
  written, and now tells the truth.
- **`host.spawn`, `host.children`, `host.events`** -- the swarm half
  of the build7 guest library. A spawn returns a handle when the
  swarm's `spawned` event lands; a denial raises with the refused
  capability in the message; `handle.kill()` awaits its outcome;
  `host.events` drains child exits, faults and budget overruns,
  buffered through any outcome wait. No `child.push`: the swarm does
  not yet deliver endpoint references between instances (build7's
  recorded cap4 gap), and the library does not pretend otherwise.
- **`script/mint_vault.lua`** -- mint a secrets vault as a compiled
  secure function. Names arrive as environment-variable names (values
  never ride argv); the output loads in a sealed guest via `load`.
  ...
  `test/test_vault.lua` proves the checks can fail on a non-secure
  build of the same table.
- **An assistant-shaped example** -- `host/assistant.host.lua` and
  `host/assistant.lua`: listener + rest + vault + spawn wired with
  honest bounds, including the raised `call_timeout_ms` an LLM round
...
`v5.5.1_build5` &middot; Lua 5.5.1 &middot; bytecode format `0x46`
...
- A crafted to-be-closed count could send record reads four
  gigabytes out of bounds on platforms with a 32-bit `unsigned
  long` (pre-existing; LP64 was never affected). Every marked
...
- **An instance had every standard library, which section 18.2 did not
  say and this release did not intend.**

  `os.execute`, `os.remove`, `io.popen`, `io.open`, `package.loadlib`,
  `require`, `dofile` and `loadfile` were all reachable by a program
  loaded into an instance; `io.open('/etc/passwd')` returned a file
  handle. `dv_new` called `luaL_openlibs`, and no section of
  `doc/Messaging.md` ever decided the standard library surface -- it
  was inherited and never looked at.
...
And Diluvium becomes a messaging runtime: a program can hold bounded
queues, park on them, and be driven by a host that moves bytes between
instances -- with a C ABI for embedding one instance, an optional
library for supervising many, and snapshot and restore underneath. The
design and its milestones are `doc/Messaging.md`; what is finished, what
is deliberately switched off, and what a deployment is accepting are in
**Known issues** below and in section 18 of that document.

Read the profiles in 18.2 before deploying this. The short version:
**hibernation is off by default and should stay off** in this release,
and the capability layer is a structuring device rather than a security
boundary while the `debug` library is available to guests. Everything
else in the messaging feature set is finished and tested.

### Added

- `msgpack`: a codec that round-trips a Lua value graph.

  Encode, decode, `as_array` and `as_map` for the shapes msgpack can
  represent but Lua cannot distinguish (an empty table is a map, not an
  array -- one of several places this bites), and `msgpack.ext` for
  application ext codes 0x10 to 0x7F. Codes below 0x10 are reserved by
  the registry in section 5.5 and are refused in both directions, so a
  program cannot write bytes only the runtime is supposed to mean
  something by.

  Integers stay integers and floats stay floats across a round trip,
  which is a property the queue layer depends on rather than a nicety.
- `queue`: bounded message queues, and a program that can park on one.

  `queue.declare`, `lookup`, `push`, `pop`, `len`, `capacity`,
  `destroy`, `disable`, and `queue.wait` over a set of them with an
  optional timeout. Queues are bounded on purpose: a full queue blocks
  the sender or fails the push, so backpressure is visible instead of
  being absorbed by memory growth.

  A program that calls `queue.wait` with nothing to read parks, and its
  host decides when it runs again. That is the whole shape of the
  runtime: no threads, no scheduler, no callbacks -- a program is a
  coroutine that stops when it has nothing to do.
- `endpoint`: a queue handle for something that is not in this state.

  A program receives an *endpoint reference* in a message and calls
  `endpoint.bind`, and what it gets back is an ordinary queue handle --
  `push`, `len`, `wait` all work unchanged. A sender cannot tell an
  endpoint from a local queue and does not need to.

  The runtime never learns what a reference means. The host resolves it,
  either through a callback or by pre-authorising bytes with
  `dv_endpoint_allow`, and then moves bytes between instances itself. A
  three-instance relay is five lines of Lua and no C on either side.
- `diluvium --task`: run a program that waits.

  The CLI grew a second entry mode, hosted on a coroutine, so a program
  that parks on `queue.wait` can be run from the command line and driven
  to completion. Ctrl-C interrupts a runaway loop in both modes.
- The instance ABI (`dv.h`, `libdiluvium`): one sandboxed instance,
  embeddable in about a dozen calls.

  `dv_new`, `dv_load`, `dv_run`, `dv_resume`, `dv_free`; queue push and
  pop by name; endpoint resolution; `dv_set_budget`, `dv_usage`,
  `dv_exceeded`; `dv_snapshot` and `dv_restore`. An instance never runs
  unbounded: a budget caps instructions and memory, and exceeding one
  stops the program with an error rather than the process with an abort.

  The header is self-contained -- no Lua header, no internal type -- and
  the contract tests are written against it alone, which is how that
  claim is kept true. Bindings for Rust (including a wasmtime host),
  Python and JavaScript ship with it, and the JavaScript one carries a
  portable WASI preview-1 host so a `.wasm` build runs in Node with no
  native dependency.
- Snapshot and restore: a running program's state as bytes.

  A parked instance can be written out and read back -- the value graph
  including cycles and shared references, closures with their upvalues,
  suspended coroutines with their frames, and content-addressed
  prototypes so a snapshot names code rather than carrying it. A
  permanents table keeps host objects out of the bytes.

  Restore treats its input as hostile: a header with a runtime
  fingerprint, structural validation, and a structure-aware fuzzer that
  mutates real snapshots and asserts a refusal rather than a crash. See
  **Known issues** for what is not finished here.
- `libdiluvium-swarm` (`dvs.h`): supervising many instances, as a
  separate library.

  An instance table, one parent link each, a capability set per
  instance, and a step function that drains each program's
  `system/lifecycle` queue and acts on what it finds -- spawn, kill,
  hibernate, query. Capabilities attenuate: a supervisor can never grant
  a child more than it holds, and `dvs_kill` takes the subtree because
  reparenting is harder to remove once something depends on it. Spawns
  are rate-limited per step, because a self-rewriting system produces a
  fork bomb eventually as a bug rather than an attack.

  There is no supervisor type, and that is the design rather than an
  omission: a supervisor is a program holding one capability. Restart
  strategies, backoff, routing and topology are ordinary Diluvium
  programs, which for a system whose programs are generated at runtime
  is the entire point.

  An app embedding a single sandbox links none of this.
- `make footprint`: what a resident instance costs, measured.

  42 KB for an instance parked on a queue -- 41 MB for a thousand, 410
  MB for ten thousand, roughly 25,000 on a 1 GB machine. Measured
  through `dv_usage` rather than RSS so the number is the same on every
  machine. It prints and asserts nothing: a threshold here would fail
  when someone adds a library rather than when something is wrong.

  This is the number to plan capacity with, and it had been guessed at
  from a snapshot's size (about 1.8 KB), which is the wrong figure once
  instances stay resident.

### Fixed

- The swarm layer's own audit found fourteen defects in it, and the four
  that a supervisor would hit first are fixed.

  A clean exit was reported to the parent as `"faulted"`, because the
  instance's last error is sticky and nothing cleared it -- so a
  supervisor restarted healthy children. Certain to happen, not a corner
  case. A kill or hibernate request narrowed a 64-bit id to 32 bits,
  which destroyed a *different live instance* instead of refusing the
  request. A spawn's code was truncated at the first zero byte, and the
  child ran a prefix while the parent was told the spawn succeeded --
  harmless for source, silent corruption for bytecode. And a budget
  written the way section 9.1 documents it, nested under `budget = {...}`,
  was ignored: the child ran unbudgeted, which let a program escape its
  own budget through a child and hang the host's step loop.

  Also: a fault's detail was read from an instance the same function had
  already freed; a capability wildcard could *widen* a grant instead of
  narrowing it; querying a hibernated instance dereferenced a null; and
  subtree kill recursed once per link where its own comment said it did
  not (measured before fixing: not reachable on any machine that could
  hold the instances, and fixed anyway so the code matches the comment).
- A reference could not be passed in a message, which is the shape a
  router has.

  A reference is an opaque object, so it went onto the wire as a plain
  one-element array: a program could use an endpoint it was handed but
  could not hand one on. The codec's resolver seam had an encode hook
  for exactly this and nothing implemented it -- and could not have, since
  the hook was only offered values the codec could not encode, and this
  one it could. Asserted now on the wire and end to end through two
  instances.
- A five-byte message could make the decoder ask for 131 MB.

  A map header claiming a huge length pre-sized a table before any
  bytes were read. Table pre-sizing is now clamped by what is actually
  left in the buffer. Found by feeding the token cursor 400,000 hostile
  inputs under AddressSanitizer, which also found an integer overflow
  that only bites where `size_t` is 32 bits -- which is wasm32, one of
  the platforms this ships for.
- `dv_resume` invented a timeout that had never elapsed.

  A host naming a queue that was live, enabled and empty was told
  `"timeout"` -- to a program that had passed no timeout, which would
  then take that branch and index the nil it was handed. It now stays
  parked and reports idle, which the host may retry. The CLI host had
  always done this correctly, so two in-tree hosts of one protocol had
  disagreed.
- Three CI jobs had been red on every run, and the tests that only ran
  there were the reason to care.

  The wasm job needed a wasmtime feature that had been renamed, the Rust
  binding pinned a wasmtime four major versions back, and a
  library-loading test needed a container it did not have. Asking the
  same question of everything else found more: no contract test had ever
  run under a sanitizer (which found undefined behaviour on its first
  run), the token cursor had never seen malformed input, four of six
  skipped tests were skipped for reasons that were factually wrong, and
  `libdiluvium-swarm` -- named as a release artifact -- was never built
  by anything. All six now run, and the swarm library's layer boundary is
  checked at the linker rather than by inspection.
...
- The bytecode loader verifies an instruction's operands against the
  ...
  One opcode needed more than the verifier. `OP_SETLIST` (and the
  vararg-table path) read a register as a table without testing it
  was one -- a dataflow property a load-time check cannot settle --
  so the VM now tests it and raises a catchable error rather than
  dereferencing a wild pointer, the way its sibling opcodes already
  do. With that, both fuzzers pass at `--allowed 0`: no single-byte
  or operand mutation crashes the release interpreter on the corpus.

  The claim this supports is that **malformed bytecode is refused or
  errors rather than crashing**. It is not that bytecode is safe: Lua
...
- An endpoint reference could be forged by a guest, two ways. One is
  closed; the other is why the capability layer is not yet a security
  boundary.

  Section 7.3 claims a reference "cannot be forged" because there is no
  constructor anywhere. There was one: `msgpack.decode` is callable by
  the program, and it ran the host's resolver on whatever bytes it was
  handed -- so a program could write `msgpack.decode('\xd4\x02' .. name)`
  and get a genuine reference to any peer the host had authorised.
  Proven end to end against a host that had authorised one peer and
  delivered nothing, where the guest reached that peer's queue. The
  reference was a guessable name, not a capability.

  Bytes that arrive from the host now resolve; bytes a program decodes
  itself surface as an inert opaque value. And they cannot be pushed
  back out to be re-decoded by the trusted path, because reserved ext
  codes are now refused at the point they would be written.

  **Not closed:** `debug.getmetatable` reaches the reference metatable
  and `debug.getregistry` reaches everything else, so a program holding
  one real reference can still mint one to any peer name it can guess.
  No scheme that keeps a reference's identity inside the runtime fixes
  that while the `debug` library is open to guests, which is why
  narrowing that library is the first item of the next profile rather
  than a patch here.
- A reserved ext code could be written from a program.

  `msgpack.ext` refuses codes below 0x10, but the wrapper it returns is
  an ordinary table: `w = msgpack.ext(0x10, s)` and then `w[3] = 0x02`
  produced exactly what the constructor had just refused. The check now
  runs where the bytes are written, because a constructor cannot vouch
  for a value that stays mutable.

  Filed as tidiness and turned out to be the security fix: it is what
  stops a decoded endpoint reference from being laundered back through a
  queue into the host's trusted delivery path.

### Known issues

- **Hibernation is switched off by default, and this release recommends
  leaving it off.**
  ...
  *restored* program unwinds from the stack base instead of the frame
  ...
  The switch exists to make the defect unreachable rather than
  documented. A deployment that keeps agent state at the application
  level and spawns a fresh instance per unit of work never enters that
  path, and pays little for it: `dv_new` plus `dv_load` of a small chunk
  is comparable to `dv_new` plus `dv_restore` of a value graph. Sizing
  for that: 42 KB per resident instance (`make footprint`).
  ...
  endpoints do not survive a snapshot, and the snapshot fuzzer's
  field-validation coverage is currently hollow -- a digest added later
  refuses every mutant before the field checks it was built to exercise,
  so "0 crashes" there is true but proves less than it appears to.
- **The capability layer is not a security boundary in this release.**

  It is a structuring device, and a good one: attenuation, subtree kill
  and per-instance budgets all work and are tested. But a program that
  holds one endpoint reference can forge another through the `debug`
  library, so a deployment must treat every program it loads as trusted
  -- written or templated by the operator, not generated by an untrusted
  party and not accepting arbitrary code from a user.

  Section 18.2 of `doc/Messaging.md` calls this profile A and states what
  closing the gap requires.
- Two smaller sharp edges worth knowing.

  Rebinding an endpoint token whose queue was destroyed returns the
  destroyed handle and poisons the token permanently -- a program should
  not `destroy` a bound endpoint queue and re-bind it. And section 10.7's
  precondition that a snapshot capture involve a single thread is
  documented but not enforced: nested coroutines are captured rather
  than refused.

### Upgrading

Nothing in the messaging work changes an existing program: the new
libraries are additions, and a program that does not call them behaves
as before. The two new libraries are opened for every state, so a
program with its own global named `queue`, `endpoint` or `msgpack`
will find it shadowed.
...
- `CHANGELOG.yaml` is now the source of truth for release notes,
  rendered by `script/changelog.py`.

  Release bodies previously carried no changelog at all -- the whole
  user-visible change list for 5.5.1_build1 was GitHub's
  auto-generated line naming one squashed pull request. That
  auto-generation is off, and the release page is now built from this
  file plus the download guide, which is derived from what a build
  produced rather than from what changed.

  `CHANGELOG.md` and `changelog.json` are generated and committed;
  CI fails when they drift, and the release mirror reads the JSON.
  The `mirror` and `latest` fields replace the mirror's
  hand-maintained tag pins.
- CI checks that the tree agrees with the newest changelog entry.

  The version is written in three places -- `VERSION`,
  `.technoproj` and `src/lua.h` -- and `LUAC_FORMAT` in a fourth.
  `script/changelog.py consistency` fails when any of them drifts
  from the entry that claims to describe them.
