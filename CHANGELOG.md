...
## [0.18.0-dev.1] - unreleased (prerelease)

`v0.18.0-dev.1` &middot; Lua 5.5.1 &middot; bytecode format `0x46`

**The development series carrying the ABI break. A host can read a parked
...
**Numbered 0.18.0 and not 0.17.0 on purpose.** This work was developed
as 0.17.0 and renumbered so that a release carrying no ABI break could
take that number: a consumer tracking this repository by git needs some
released version between 0.16.0 and the break, and there was none. The
`v0.17.0-dev.1` and `v0.17.0-dev.2` prereleases were cut from this branch
under its old number and describe this entry, not 0.17.0.

This entry is the one a `-dev.` build renders. It stays `unreleased`
until 0.18.0 ships, at which point its `version` and `tag` lose the
...
- **This does not reach DRT until 0.18.0 ships.** The work lives on
  `release/0.18.0` and is cut as dev tags from there;
