...
**The instance ABI, found after the sweep.**

| Defect | Consequence |
|---|---|
| ~~The instruction budget was switched off by its own first firing (`dv.c`)~~ **Fixed.** | The count hook cleared itself before raising, and `luaL_error` raises an ordinary *catchable* error — so a guest's own `pcall` caught it and nothing re-armed the hook (`dv_run` and `dv_restore` are the only other sites that arm it, and neither is reachable again on a running instance). `pcall(function() while true do end end)` followed by any loop ran unbounded for the life of the instance. Measured: a 1,000,000 budget, 30,000,000 instructions of work, `dv_run` returning `DV_DONE`. `insn_used` advances only in the hook, so `dv_usage` froze at exactly the limit — an escaped instance read as the healthiest possible instance, which blinded the one measurement a supervisor would have used. The hook stays armed now; `a_budget_survives_a_guest_pcall` and `usage_keeps_counting_past_the_budget` hold both halves, and both were verified red first. |
| **A guest that catches the budget error in a loop still pins a core.** Open, and a decision rather than a defect to patch. | The fix above bounds the *work* a guest can do past its budget. It does not make the error uncatchable and does not return control to the host: `while true do pcall(function() while true do end end) end` never leaves `dv_run` — each catch buys `DV_HOOK_STEP` instructions and the loop repeats. Measured against the fixed build: still spinning at 25 s. Lua has no uncatchable error, and every option inside the hook produces one — a smaller `DV_HOOK_STEP` shrinks the slice and still spins, and yielding is ruled out on the record above `dv_insn_hook` (a hook yield leaves `CIST_HOOKYIELD` and costs hibernation). Upstream Lua's own interpreter has the same shape in `lua.c`'s `lstop`, with a second SIGINT as its backstop; there is no equivalent here, because `dv.h` exposes no interrupt for a host already inside `dv_run`. Two ways to close it, both decisions: a `pcall`/`xpcall` that refuses to catch once `exceeded` is set — `lbaselib.c` is not in `CORE_PATCH_ALLOWLIST`, and `patch_series.sh` calls adding an entry "the review point" — or a process-level watchdog. Adding `dv_interrupt()` does **not** substitute: the error it raised would be catchable too, so the same loop absorbs it on a different clock. Not reachable by input alone, unlike the escape above: it needs code *written* as a catch-loop, so a deployment that signs and verifies its programs has addressed the authorship half already. |
