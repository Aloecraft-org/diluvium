--[[
ATTIC. A runnable sketch, not a foundation, and not built on the shipped runtime.

Salvaged from a branch that forked before the messaging work landed. It models a
deterministic actor scheduler with coroutines in one Lua state; the tree's real answer
is an instance per actor (dv_new), so treat the mechanism here as an argument about
*ordering* and not as an example of how to write Diluvium programs. For that, see
doc/Guide.md and examples/discofetch.

The argument it exists to support is doc/Determinism.md. This file's value is that it
runs and proves its own claim -- it executes a small map-reduce swarm twice and asserts
the delivery traces are byte-identical:

    ./dist/diluvium_linux_x86_64 doc/attic/swarm-prototype.lua

What it does NOT do: the distributed tier or any ordering authority (one counter), real
per-actor isolation, preemption or budgeting (actors are trusted to yield), or
defer/with cancellation semantics.
]]
