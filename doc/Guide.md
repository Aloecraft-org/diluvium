...
**An instance is not a sandbox in this release.** Measured, not assumed — a program
loaded into a fresh `dv_new(NULL)` instance reports:

```
os.execute=true io.popen=true os.remove=true os.exit=true
debug.getregistry=true debug.sethook=true package.loadlib=true
```

So a program you load can start a process, read and write files, and switch off its own
instruction budget by taking the one `lua_sethook` slot. `dv_new` gives an instance the
standard library set, and the standard set reaches outside the state. The isolation an
instance really gives you today is **memory isolation between instances** — separate
`lua_State`s, separate heaps, a budget on each — and that is what makes one-instance-per-
client worth doing. It is not a boundary against the program itself.

Two flags to narrow this are in flight and not yet on `main`: one to narrow `debug`, one
to leave `io`, `os` and `package` out. Until they land, run only programs you wrote or
templated. See part 9.
...
**An instance is not a boundary around the program in it.** This is the biggest caveat
in this file and the one to read twice. A program in a fresh instance has `os.execute`,
`io.popen`, `io.open`, `package.loadlib` and the whole `debug` library — verified by
running it, see part 6. It can therefore start a process, touch the filesystem, forge an
endpoint reference through `debug.getmetatable`, and disable its own instruction budget
via `debug.sethook`. Attenuation, subtree kill and budgets all work and are tested, but
they constrain what a program is *given*, not what it can *reach*.

**So treat every program you load as trusted** — written or templated by you, not
generated by an untrusted party and not accepting arbitrary Lua from a user. That is
§18.2's profile A and it is the release's supported configuration. What you do get is
isolation *between* instances: separate states, separate heaps, a budget each, and a
kill that takes the subtree.

Narrowing both is in progress on a branch, as two opt-in flags rather than a change of
default, and whether an instance should be sealed *by default* is an open decision on
§18.3's checklist. When that lands this section changes; until then it is accurate.
