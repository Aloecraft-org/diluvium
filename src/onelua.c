...
/* Since Lua 5.5, ltests.c and lstate.c each define a private static
   'resetCI'. That is fine across separate translation units but clashes
   in this amalgamation; rename ltests.c's copy (the Lua-visible name in
   its registration table is a string literal and is unaffected). */
#define resetCI resetCI_ltests
#include "ltests.c"
#undef resetCI
#endif
