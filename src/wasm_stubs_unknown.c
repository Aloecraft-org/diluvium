...
__attribute__((export_name("repl_eval"))) int repl_eval(const char *code)
{
    int st;
    if (global_L == NULL)
        init_lua();
...
*/
__attribute__((export_name("repl_complete"))) char *repl_complete(const char *buf, int cursor)
{
    size_t off, need = 24;
    int i, n;
    char *out, *p;
...
__attribute__((export_name("reset_lua"))) void reset_lua(void)
{
    if (global_L != NULL) {
        lua_close(global_L);
        global_L = NULL;
